...
//    public long root;
    ...
    public long[] filter(int[] indexs) throws Exception {
        if (indexs.length == 0) {
        ...
        for (int index : indexs){
            minLength = Math.min(minLength, bitmaps[index].length);
        }

        long[] result = new long[minLength];
        Arrays.fill(result, allBitsOne);

        long[][] selectedBitmap = new long[indexs.length][];
        for (int i = 0; i < indexs.length; i++) {
            selectedBitmap[i] = bitmaps[indexs[i]];
        }

        return IntStream.range(0, minLength).parallel().mapToLong(j -> {
            long k = allBitsOne;
            for (int i = 0; k!=0 && i < indexs.length; i++)
                k = k & selectedBitmap[i][j];
            return k;
        }).toArray();

    }

    public long[] filter2(int[] indexs) throws Exception {
        if (indexs.length == 0) {
            long[] result = new long[this.bitmaps[0].length];
            Arrays.fill(result, allBitsOne);
            return result;
    ...
    public static BIT createIndex(List<long[]> data, long rootId) throws Exception {
        ...
        long beforeEncoding = System.currentTimeMillis(); // CHECKPOINT
        ...
        System.out.println("Encoding time: " + (System.currentTimeMillis()-beforeEncoding));
        long beforeBIT = System.currentTimeMillis(); // CHECKPOINT
        ...
        System.out.println("BIT time: " + (System.currentTimeMillis()-beforeBIT));
        return bit;
    }
